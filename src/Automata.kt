...
//    Parse methods
                ...
                if (currentState.hasTransitions()) {
                    alphabet.forEach { symbol ->
                        ...
                        if (currentTransitions.isNotEmpty() && currentTransitions.count() > 1) {
                            ...
                            newState.final = currentState.final
                            currentTransitions.forEach { t ->
                                newTransitions.addAll(t.state.getTransitions())
                                currentState.removeTransition(t)
                                newState.final = t.state.final || newState.final
                            }
                            newState.addTransitions(newTransitions)
                            currentState.newTransition(symbol, newState)
                        }
                    }
                }
            }
    ...
    fun removeDeadsAndUnreachables() {
